                        ...
                        {expandedItem === agent.id && (
                            <div className="p-4 border-t border-gray-300 dark:border-green-400 bg-gray-50 dark:bg-gray-900">
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <div className="text-sm text-gray-500 dark:text-green-600 mb-2">Recent Usage:</div>
                                        {agent.recentUsage.length > 0 ? (
                                            <div className="space-y-2">
                                                {agent.recentUsage.map((usage) => (
                                                    <div key={usage.id} className="text-sm flex justify-between">
                                                        <span>{usage.type}</span>
                                                        <span className="text-gray-500 dark:text-green-600">{usage.timestamp}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        ) : (
                                            <div className="text-sm text-gray-500 dark:text-green-600">No recent activity</div>
                                        )}
                                    </div>

...
export function initCommonMiddlewares(app: Hono<HonoEnv>) {
  ...
  app.use(async (c, next) => {
    ...
    const verifiedAddress = await ethers.verifyMessage('You are logging into SecretAgent.sh', authMessage);
...
export const loggedInOnly = createMiddleware<LoggedInHonoEnv>(async (c: Context<LoggedInHonoEnv>, next) => {
  if (!c.var.authUserId) {

...
export const projectsTable = sqliteTable("projects", {
  ...
  ownedByUserId: text("owned_by_user_id").notNull().references(() => usersTable.id),
  createdAt: text('created_at').notNull().default(sql`(CURRENT_TIMESTAMP)`),
});

export const configItemsTable = sqliteTable("config_items", {
...
export const projectAgentsTable = sqliteTable("project_agents", {
  ...
  label: text("label"),
  createdAt: text('created_at').notNull().default(sql`(CURRENT_TIMESTAMP)`),
  status: text('status', { enum: ['pending', 'enabled', 'paused', 'disabled'] }).notNull().default('pending'),
}, (table) => [
  // compound primary key
